...
export function syncTopRatedRanking(rows: RankedRestaurantRow[]): void {
...
export function isTopRatedRankingCacheFresh(): boolean {
  const stored = readStored();
  if (!stored || stored.restaurantIds.length === 0) return false;
  return Date.now() - stored.updatedAt < MAX_AGE_MS;
}
